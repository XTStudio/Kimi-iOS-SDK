...
- (void)testGET {
    ...
    [context evaluateScript:@"URLSession.shared.dataTask('https://httpbin.org/get', function(data, response, error){ if (JSON.parse(data.utf8String()).url === 'https://httpbin.org/get'){ sessionGetTest.fulfill() } }).resume()"];
...
- (void)testGETRequestWithURLString {
    ...
    KimiTestExpectation *sessionGetTest = [[KimiTestExpectation alloc] init];
    [AppDelegate setUnitTestObject:sessionGetTest forKey:@"sessionGetTest"];
    [context evaluateScript:@"var req = new MutableURLRequest('https://httpbin.org/get');"];
    [context evaluateScript:@"URLSession.shared.dataTask(req, function(data, response, error){ if (JSON.parse(data.utf8String()).url === 'https://httpbin.org/get'){ sessionGetTest.fulfill() } }).resume()"];
    [self waitForExpectations:@[sessionGetTest] timeout:10];
}
...
- (void)testPOST {
    ...
    [context evaluateScript:@"var req = new MutableURLRequest(URL.URLWithString('https://httpbin.org/post')); req.HTTPMethod = 'POST'; req.setValueForHTTPHeaderField('application/x-www-form-urlencoded', 'Content-Type'); req.HTTPBody = new Data({utf8String: 'username=PonyCui'})"];
...
- (void)testStatusCode {
    ...
    [context evaluateScript:@"URLSession.shared.dataTask('https://httpbin.org/status/233', function(data, response, error){ if (response.statusCode === 233){ sessionStatusCodeTest.fulfill() } }).resume()"];
